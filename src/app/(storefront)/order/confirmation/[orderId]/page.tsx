...
export default function OrderConfirmationPage() {
    ...
    const handleReorder = async () => {
        ...
        try {
            ...
            if (!activeCartId) {
                await initGuestCart();
                activeCartId = useCartStore.getState().cartId ?? null;
            }
    ...
    const pendingTooLong =
        isPendingPayment &&
        now - new Date(order.createdAt).getTime() > 2 * 60 * 60 * 1000;
                ...
                <section className="rounded-lg border bg-card p-6">
                    ...
                    <ol className="mt-4 space-y-4">
                        {statusTimeline.map((item, index) => {
                            ...
                            const statusLabel =
                                ORDER_STATUS_LABELS[statusKey]?.label ??
                ...
                <section className="rounded-lg border bg-card p-6">
                    ...
                    <div className="mt-3 space-y-2 text-sm text-muted-foreground">
                        {order.shipping ? (
                            <>
                                <p>
                                    Kurir: <span className="font-medium text-foreground">{order.shipping.courier}</span>
                                </p>
                                <p>
                                    Layanan: <span className="font-medium text-foreground">{order.shipping.service}</span>
                                </p>
                                {order.shipping.trackingNumber ? (
                                    <p>
                                        Nomor resi:{' '}
                                        <span className="font-medium text-foreground">{order.shipping.trackingNumber}</span>
                                    </p>
                                ) : (
                            ...
                            {order.items.map((item) => (
                                <li key={item.id} className="flex flex-col gap-3 rounded-lg border p-4 sm:flex-row">
                                    ...
                                    <div className="flex-1 space-y-1">
                                        ...
                                        <p className="text-xs text-muted-foreground">
                                            Qty {item.qty} - {formatCurrency(Number(item.unitPrice) || 0, order.currency || 'IDR')}
                                        </p>
                                    ...
                                    <div className="flex flex-wrap gap-2 sm:flex-col sm:items-end">
                                        ...
                                        <Button asChild variant="ghost" size="sm">
                                            <Link href={`/products/${item.productSlug}#reviews`}>
                                                Beri ulasan
                                            </Link>
                                        </Button>
                                    </div>
                ...
                onConfirm={async (reason, note) => {
                    try {
                        ...
                        toast({
                            ...
                            description: 'Pesanan Anda sudah dibatalkan. Tim kami akan memproses pengembalian dana jika diperlukan.',
                        });
                        setCancelOpen(false);
                    } catch (error) {
                        toast({
