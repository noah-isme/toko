...
- `kind` — `fixed_amount` or `percent`
- `percentBps` — basis points for percent vouchers (e.g. `2000` = 20%)

**Response:** `201 Created` — `{ "data": { ...voucher object... } }`
...
Updates the voucher identified by its code. Uses the same request shape as creation.

**Response:** `200 OK` — `{ "data": { ...voucher object... } }`

Common errors: `NOT_FOUND`, `BAD_REQUEST`.
...
Dry-run evaluation of a voucher without persisting state.
...
```json
{
  "data": { "discount": 50000, "eligible_amount": 250000, "code": "DISC20" }
}
```

Common errors: `NOT_ELIGIBLE` (voucher cannot be applied).
...
Common errors: `INVALID_STATE` (transition not allowed), `NOT_FOUND`.
...
Common errors: `INVALID_STATE`, `ALREADY_EXISTS`, `NOT_FOUND`.
...
Returns configured webhook endpoints. Query parameters: `limit` (default `50`, max `200`), `offset` (default `0`).

**Response:** `200 OK` — `{ "data": [ ...endpoint objects... ] }`
...
Returns webhook delivery attempts with optional filtering by `endpointId`, `eventId`, and `status`.

**Response:** `200 OK` — `{ "data": [ ...deliveries... ], "total": 42 }`
...
{
  "replayed": ["dlq-uuid-1"],
  "failed": { "dlq-uuid-2": "invalid payload" }
}
```
...
Returns queue depth, in-flight count, DLQ size, and oldest lag for a given queue kind. `kind` is required.
...
Returns a paginated list of audit logs. Query parameters: `limit` (default `50`, max `200`), `offset` (default `0`).

**Response:** `200 OK` (raw array, not wrapped in `data`)
...
Common errors: `AUDIT_NOT_CONFIGURED`, `AUDIT_QUERY_FAILED`.
